...
class XMLHandler: NSObject, XMLParserDelegate {
        ...
        if !skip {
            ...
            if elementName == "abstract" {
                ...
                if attributeDict["type"] == "figure" {
                    ...
                    appendFigureToParagraph(figureAttributes: attributeDict)
                }
                if attributeDict["type"] == "table" {
    ...
    func appendFigureToParagraph(figureAttributes: [String : String]) {
        
        guard let preProcessedName = figureAttributes["target"] else { return }
        
        guard let convertedName = convert(input: preProcessedName) else {return}
        
//        guard let location = figureAttributes["coords"] else {return}
//
//        let locationURLCode = location.replacingOccurrences(of: ",", with: ":")
        
        
        
        
        currentObject.div[divIndex].p[pIndex].value.append(" [\(convertedName)](\(preProcessedName)) ")
